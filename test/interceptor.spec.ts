...
describe('interceptors', () => {
  ...
  test('should add arequest interceptor', () => {
  ...
  test('should add a response interceptor', done => {
    ...
    instance.interceptors.response.use(response => {
      response.data = response.data + ' - modified by inteceptor'
      return response
    })
    ...
    getAjaxRequest().then(request => {
      ...
      setTimeout(() => {
        expect(response.data).toBe('OK - modified by inteceptor')
        done()
      }, 100);
    })
  })
  ...
  test('should allow removing interceptors', done => {
    ...
    intecept = instance.interceptors.response.use((response) => {
      response.data = response.data + '2'
      return response
    })
    instance.interceptors.response.use((response) => {
    ...
    instance.interceptors.response.eject(intecept)
