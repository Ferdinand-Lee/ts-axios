...
import { type } from "os";
function getSpy(): [(res: AxiosResponse) => AxiosResponse, (e: AxiosError) => AxiosError] {
  return [
    jest.fn((res: AxiosResponse) => {
      return res
    }),
    jest.fn((e: AxiosError) => {
      return e
    })
  ]
}
describe('requests', () => {
  ...
  test('should reject on network errors', () => {
    ...
    function next(reason: AxiosResponse | AxiosError) {
      ...
      expect(reason.request).toEqual(expect.any(XMLHttpRequest)) // 断言是XMLHttpRequest 类型的实例
      jasmine.Ajax.install()
    }
  })
  test('should reject when request timeout', done => {
    ...
    getAjaxRequest().then(request => {
      // @ts-ignore
      request.eventBus.trigger('timeout') // request.responseTimeout()
      setTimeout(() => {
  ...
  test('should reject when validateStatus returns false', () => {
    const [resolveSpy, rejectSpy] = getSpy()
    axios('/foo', {
